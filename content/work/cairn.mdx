...
The engine is not the renderer. A tooltip library dies when the DOM changes. A state machine survives navigation. `next` is a function of live context — an [index is not a path](/writing/an-index-is-not-a-path).
