...
I already knew the failure. If research and edit are two products, they drift. If gen and regen are two products, they drift. I wrote that on this site before I hired anyone. *One spine.* The brief that starts the work has to be the brief that reviews it.

One Bot with four hats is the same failure with a friendlier name. It will write the admin panel and the signed PDF and the Linear ticket and call that a day. The copy forks. The hole gets filled.

I have more than one product. **Psynth** is a clinical record. **Dripnex** is an offline notes app. **DolarGaucho** is a quotes site. This site is files in git. A Dripnex Bot that can see Linear PSY will file the wrong ticket. A facts Bot that can invent an EMBI print will invent one. The negative on the profile is how I stop that.

I created **Bot Organicer** *after* the first names, not before. Overlap is two Bots who think they own the same panel. The organizer inventories, cuts the double, writes the description. *It does not write product code.* I cannot hold thirty-five jobs in my head. The profile is the memory.

I have not pointed this public pair at the signed report. Naming the jobs is how I refuse to put a Bot on that path. The clinician still signs. The product still has to make being wrong visible.

## Why I automate

A Bot is a computer that keeps running when the lid closes. That is the product I hired. Not a faster chat.

Some jobs are a contract on a clock. **Dripnex Themes** writes exactly three official palettes a day into `officialThemes.ts`, logs them in `docs/themes/LOG.md`, so the next run never repeats. I should not have to remember that on a Saturday.

**Code fixer** takes every @CodeRabbit comment, applies that fix, leaves the rest of the PR alone. Mechanical. If I do it by hand I will start "improving" the file.

This stream has a weekday pull at 8:08 ART. If there is no commit, no Bot decision, and no file, it stays silent. Empty is cheaper than a sentence I did not earn.

**Bot Organicer** has a P0 pulse weekdays at 9:00 ART. It only speaks if something is stuck. Head of Product decides only if that P0 needs a decision. Head of Engineering executes the cut. Product Manager does not open new tickets from a pulse.

I automate the jobs that should not wait for me to remember them. I do not automate the report someone signs. I do not automate a post. I still read the draft and the still.

## How they coordinate

The profile is the API. **Owns X.** *Does not own Y.* That is the handoff. Generation does not own PDF. Export does not invent clinical content. Ingest does not own the EHR. Integrations does not own the upload path. App Engineer does not own the design system. Head of Engineering does not implement.

For Psynth the sequence is written down. **Head of Product** decides. **Head of Engineering** says who. **Product Manager** puts it in Linear, team PSY. Linear is the work. The organizer does not invent a second queue.

Rooms are not jobs. **Leadership**, **Build**, **Quality**, **Delivery**, **Admin**, **Ambient**, **GTM**, **Dripnex** are member lists so a discussion has a place. They do not own a surface. I also left empty shells with those names. Coordination is incomplete. **Plugin Innovation** exists and is *not* in the Dripnex room.

They share one computer and its logins. They can hand a file without a zip. They also share cookies. A named crew is *not* a second security boundary. Each Bot gets a screen. They do not get a second cookie jar. That is why a production admin session does not live on that VM "just to try."

This pair is narrower. **Redactor** drafts. **Portfolio** drives Playwright and cuts a 16:9 capture of a real product. I approve. Neither posts. **DolarGaucho** checks a number before it goes into a sentence. If it cannot see the figure it says so.

They message each other when a draft needs a still, or a still needs a fact. I am the last mile. That is the cost I accepted: a slower day, and a roster I have to keep.
